...
REPEAT = 10
DISCARD_EXTREMES = 2
MESSAGES_SIZES = list(range(32)) + [2 ** exp for exp in range(5, 14)]
FRAMES_COUNT = 10
MESSAGES_COUNT = 100
...
for messages_size in MESSAGES_SIZES:
    ...
    results.append((messages_size, azmq_durations, pyzmq_durations))
...
data = [
    Scatter(
        name='azmq',
        x=[messages_size for messages_size, _, _ in results],
        y=[mean(durations) for _, durations, _ in results],
        error_y=dict(
            type='data',
            symmetric=False,
            array=[
                max(durations) - mean(durations)
                for _, durations, _ in results
            ],
            arrayminus=[
    ...
    Scatter(
        name='pyzmq',
        x=[messages_size for messages_size, _, _ in results],
        y=[mean(durations) for _, _, durations in results],
        error_y=dict(
            type='data',
            symmetric=False,
            array=[
                max(durations) - mean(durations)
                for _, _, durations in results
            ],
            arrayminus=[
                mean(durations) - min(durations)
                for _, _, durations in results
